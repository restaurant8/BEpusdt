...
| 配置项      | 说明                                                         |
|----------|------------------------------------------------------------|
| **镜像**   | 正式版：`ghcr.io/restaurant8/bepusdt:latest` / 开发版：`ghcr.io/restaurant8/bepusdt:nightly` |
| **端口**   | 反向代理场景：按图配置；外网暴露：两个端口均填 `8112` 并勾选"对外暴露"                   |
| **重启规则** | 无特殊需求时按图中默认配置                                              |
...
若已对外暴露 8112 端口，访问 `http://<服务器IP>:8112` 可查看初始化页面。
